...
async function openAIChat(chatId) {
    try {
        ...
        // 对所有 AI 回复气泡兜底渲染 LaTeX 公式
        $$('.ai-bubble', $('#ai-messages')).forEach(bubble => {
            bubble.removeAttribute('data-latex-rendered');
...
function renderAiMessage(role, content) {
    ...
    if (role === 'assistant' && typeof marked !== 'undefined') {
        ...
        try {
            // 将 LaTeX 的 \(...\) 和 \[...\] 转为 marked math 扩展识别的 $...$ / $$...$$
            const preprocessed = content
                .replace(/\\\(([\s\S]*?)\\\)/g, '$$$1$$')
                .replace(/\\\[([\s\S]*?)\\\]/g, '$$$$$1$$$$');
            renderedContent = marked.parse(preprocessed);
        } catch (e) {
            ...
            if (typeof marked !== 'undefined') {
                try {
                    const preprocessed = reply
                        .replace(/\\\(([\s\S]*?)\\\)/g, '$$$1$$')
                        .replace(/\\\[([\s\S]*?)\\\]/g, '$$$$$1$$$$');
                    lastBubble.innerHTML = marked.parse(preprocessed);
                    // 兜底渲染：处理 marked 扩展可能遗漏的 $...$ 公式
                    renderLatexInElement(lastBubble);
                } catch (e) {
